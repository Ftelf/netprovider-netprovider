...
class Mail_mimeDecode extends PEAR{
    ...
    var $_crlf;
    /**
     * Variable to hold output encoding.
     * @var    string
     */
    var $_output_encoding;
    ...
    public function __construct($input, $crlf = "\r\n")
    {
        ...
        $this->_output_encoding = isset($params['output_encoding'])  ? $params['output_encoding']  : "UTF-8";
    }
    ...
    function decode($params = null)
    {
        ...
        if ((!isset($this) OR !($this instanceof Mail_mimeDecode)) AND isset($params['input'])) {
        ...
        } elseif (!isset($this) OR !($this instanceof Mail_mimeDecode)) {
            return 'Called statically and no input given';

        // Called via an object
        } else if ($this instanceof Mail_mimeDecode) {
    ...
    function _decode($headers, $body, $default_ctype = 'text/plain')
    {
        ...
        if (isset($content_type)) {
            switch (strtolower($content_type['value'])) {
                case 'text/plain':
                    $this->_include_bodies ? $return->body = ($this->_decode_bodies ? $this->_decodeBody($body, $return->ctype_parameters['charset'], $content_transfer_encoding['value']) : $body) : null;
                    break;
            ...
            $this->_include_bodies ? $return->body = ($this->_decode_bodies ? $this->_decodeBody($body, $return->ctype_parameters['charset']) : $body) : null;
        }
    ...
    function _parseHeaderValue($input)
    {
        if (($pos = strpos($input, ';')) !== false) {
            ...
            if (strlen($input) > 0) {
                preg_match_all('/(([[:alnum:]]+)="?([^"^\s^;]+)"?\s?;?)+/i', $input, $matches);
     ...
     * @access private
     */
    function _decodeBody($input, $charset, $encoding = '7bit')
    {
        switch ($encoding) {
            case '7bit':
                return iconv($charset, $this->_output_encoding, $input);
                break;

            case 'quoted-printable':
                return iconv($charset, $this->_output_encoding, $this->_quotedPrintableDecode($input));
                break;
