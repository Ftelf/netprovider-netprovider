...
| **D3** | `proceedChargesForPerson`, `ChargesUtil.php:368` | ✅ **FIXED** — `personaccount` is now written only on the successful-collection path (where the balance actually moved); the insufficient-funds path persists only the `chargeentry`. | One account write only when the balance changes. The doc's original "single write after the walk" was **not** adopted: it would break the per-entry (account+entry) atomicity that D2 relies on. | Resolved: no redundant no-op account write; money-integrity atomicity preserved. |
| **D4** | `removeChangeEntriesOutOfScope`, `ChargesUtil.php:215` | ✅ **FIXED** — an open (`null`) `dateEnd` now renders as `∞` in the removal log line instead of the empty string `getFormattedDate()` returns. | Format an open end as "∞". | Log/UI cosmetic only. |
| **D5** | `proceedChargesForPerson`, `ChargesUtil.php:399-425` | ✅ **FIXED** — bitwise `&=` replaced with explicit `$flag = $flag && <verdict>` (PHP has no `&&=`). Every status is kept as its own branch and assigns its verdict as a visible literal/expression (`&& true`, `&& false`, `&& (overdue <= tolerance)`), so the code mirrors the §5.4 table 1:1; unmatched statuses (e.g. `ERROR`) stay neutral. Flags stay `bool`. | Explicit boolean logic, one branch per status. | Resolved: type-correct, no bitwise on logical values; behavior unchanged (covered by the §5.5 decision-table tests). |
...
5. **D3/D4/D5 — low-priority cleanups: FIXED** (2026-09-12). D3: account persisted only when the balance moves (per-entry account+entry atomicity kept; the "single deferred write" idea was rejected as it would undo D2). D4: open `dateEnd` logs as `∞`. D5: bitwise `&=` → explicit boolean logic, flags stay `bool`. No behavior change for D4/D5; D3 removes the redundant no-op write on the insufficient-funds path (`testProceedChargesForPersonMarksInsufficientFundsWhenBalanceLow` updated).
